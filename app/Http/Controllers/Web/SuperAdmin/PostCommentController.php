...
class PostCommentController extends Controller
{
	...
	*/
    ...
    public function index(Request $request){
		...
		$data['page_js'] = array(
            'backend/assets/superadmin/js/post_comment.js'
        );
        ...
        $totalRecords = PostComment::with('user:id,user_name,role')
		...
		if ($request->ajax()) {
            ...
            return view('superadmin.posts.comments.list',$data);
        }
        else
        {
            Alert::info('Comment not found.',  'No any comment found for this post.', 'info');
		    return redirect('superadmin/posts');
        }
    }
	...
	*/
	public function addComment(Request $request){
		...
		$row['postId'] = $postId;
		return view('superadmin.posts.comments.add',$row);
	}
	...
	*/
    public function commentReplyForm(Request $request){
		...
		return view('superadmin.posts.comments.reply',$row);
	}
    ...
    */
    ...
    public function viewCommentUpVotes(Request $request){
        ...
        $upVotes = CommentsUpvoteAndDownvote::with('user')
    ...
    */
    ...
    public function viewCommentDownVotes(Request $request){
        ...
        $downVotes = CommentsUpvoteAndDownvote::with('user')
             ->where('status', 0)
             ->where('comments_id',$id)
             ->orderby('created_at','desc')
             ->get();
    ...
    */
    public function view(Request $request){
        ...
        $row['data'] = PostComment::with('user:id,user_name,role')
        ->withCount('commentLike')
        ->withCount('commentDownVote')
        ->withCount('flaggedComment')
