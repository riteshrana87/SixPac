...
class ConsumerUserController extends Controller
{
    public $dataTable;
	public function __construct()
    {
        ...
        $this->userImportPath = Config::get('constant.IMPORT_CONSUMER_USER_CSV_UPLOAD_PATH');
        $this->userExportPath = Config::get('constant.EXPORT_CONSUMER_USER_CSV_UPLOAD_PATH');
        $this->fileSystemCloud = Config::get('constant.FILESYSTEM_CLOUD');
        $this->dataTable = new DataTableService();
    }
	/*
		@Author : Spec Developer
		@Desc   : Fetch consumer user listing
		@Output : \Illuminate\Http\Response
		@Date   : 02/03/2022
	*/
    public function index(Request $request){
    	$data['page_title'] = 'Consumer Users';
		$data['page_js'] = array(
            'backend/assets/superadmin/js/consumer_users.js'
        );
        ...
        if ($request->ajax()) {
            $select = ['id',DB::raw('IF(name!="" AND name IS NOT NULL,name,"-") AS name'),'email','phone','created_at','role','status'];
            $query = User::whereNotIn('id', [Auth::user()->id])
            ->where(['role' => 5]);
            $search = ['name', 'email', 'phone'];
            $actions = [
                'view' => url('superadmin/users/consumer-users/view'),
            ];
            $this->dataTable->showTable($request,$query,$select, $search, $actions, 1);
        }
        $filename = 'consumer_users.csv';
        $data['csvFile'] = Storage::disk(Config::get('constant.FILESYSTEM_CLOUD'))->url(Config::get('constant.EXPORT_CONSUMER_USER_CSV_UPLOAD_PATH').$filename);
        return view('superadmin.users.consumer_user_list',$data);
    }

	/*
		@Author : Spec Developer
		@Desc   : Add consumer user.
		@Output : \Illuminate\Http\Response
		@Date   : 28/02/2022
	*/
    public function add(){
		$data['page_title'] = 'Add Consumer User';
        $data['page_js']    = array(
            'backend/assets/superadmin/js/consumer_users.js'
        );
        $data['extra_css'] = array(
        ...
        return view('superadmin.users.add_consumer_user',$data);
	}

	/*
		@Author : Spec Developer
		@Desc   : Store new consumer user data.
		@Output : \Illuminate\Http\Response
		@Date   : 02/03/2022
	*/
    public function store(Request $request){
       ...
       try {
            ...
            if ($validator->fails()) {
                Log::info('Add consumer user by super admin :: message :' . $validator->errors());
            ...
            Alert::success('Success', 'Consumer user has been added!.', 'success');
		    return redirect('superadmin/users/consumer-users');
        } catch (\Exception $e) {
            // Log error message
		    ...
		    return redirect('superadmin/users/consumer-users/add');
        }
    }

    /*
		@Author : Spec Developer
		@Desc   : Edit consumer user.
		@Output : \Illuminate\Http\Response
		@Date   : 02/03/2022
	*/

    public function editConsumerUser($id){

        $data['page_title'] = 'Edit Consumer User';
        $data['page_js']    = array(
            'backend/assets/superadmin/js/consumer_users.js'
        );
        $data['extra_css'] = array(
		...
		$results['countryArr'] = Country::select('countries.id as id','countries.name as name')
        ...
        $results['stateArr'] = array();
        $results['cityArr'] = array();

        if(!empty($data['country'])){
            $countryId = $data['country'];

            $results['stateArr'] = Country::select('states.id as id','states.name as name')
                ->join('states','states.country_id','=','countries.id')
                ->join('cities','cities.state_id','=','states.id')
            ...
            $results['cityArr'] = City::select('id','name')
                ->where(['country_id' => $countryId,'state_id' => $results->consumer->state])
                ->orderby('name','ASC')
                ->get();
        }

        $data['data'] = $results;
        return view('superadmin.users.edit_consumer_user',$data);
    }

    /*
        @Author : Spec Developer
        @Desc   : Update consumer user details.
        @Output : \Illuminate\Http\Response
        @Date   : 02/03/2022
    */

	public function updateConsumerUser(Request $request){
        ...
        try {
            ...
            $validator = Validator::make($request->all(),$validationArr);
            if ($validator->fails()) {
                Log::info('Edit consumer user by super admin :: message :' . $validator->errors());
                return redirect()->back()->withErrors($validator)->withInput();
            }
           ...
           ConsumerProfileDetail::updateOrCreate(
            ...
            Alert::success('Success', 'Consumer user details updated.', 'success');
		    return redirect('superadmin/users/consumer-users');
        } catch (\Exception $e) {
            // Log error message
            Log::error(strtr(trans('log-messages.DEFAULT_ERROR_MESSAGE'), [
    ...
    /*
        @Author : Spec Developer
        @Desc   : View consumer user details.
        @Date   : 02/03/2022
    */
    ...
    public function view(Request $request){
        ...
        $data['data'] = $results;
        return view('superadmin.users.view_consumer_user',$data);
    }

    /*
        @Author : Spec Developer
        @Desc   : Delete consumer user record.
        @Output : \Illuminate\Http\Response
        @Date   : 22/02/2022
    */
    ...
    public function destroy($id){
        ...
        Alert::success('Success', 'Consumer user has been deleted successfully!', 'success');
		return redirect('superadmin/users/consumer-users');
    }

	/*
		@Author : Spec Developer
		@Desc   : Change staus of consumer user.
		@Output : \Illuminate\Http\Response
		@Date   : 11/03/2022
	*/
	...
	*/
	...
	public function importConsumerUsers(Request $request){
		...
		if ($file) {
            ...
            if(in_array(strtolower($extension),$valid_extension)){
                ...
                if($fileSize <= $maxFileSize){
                    ...
                    $filePath = Storage::disk(Config::get('constant.FILESYSTEM_CLOUD'))->path(Config::get('constant.IMPORT_CONSUMER_USER_CSV_UPLOAD_PATH'));
                    ...
                    foreach($importData_arr as $importData){
                        ...
                        if ($countUser) {
                            ...
                            $input['date_of_birth']    = trim($importData[3]); //$dob - Y-m-d;
                            $input['gender']    = $gender;
                            $input['updated_by']    =   1;
                            ...
                            $obj = new User([
                                ...
                                "phone" =>  $phoneNumber,
                                "date_of_birth" =>  trim($importData[3]), //$dob,
                                "gender"        =>  $gender,
                                "created_by"    =>  1,
                                "role"          =>  $role,
                                "status"        =>  1,
                        ...
                        if(!empty($userId)){
							$address    = (!empty(trim($importData[5])) || trim($importData[5]) != '-') ? trim($importData[5]) : NULL;
                            $unitApt    = (!empty(trim($importData[6])) || trim($importData[6]) != '-') ? trim($importData[6]) : NULL;

							$city    = (!empty(strtolower(trim($importData[7]))) || trim(strtolower(trim($importData[7]))) != '-') ? trim($importData[7]) : NULL;
							$state    = (!empty(strtolower(trim($importData[8]))) || trim(strtolower(trim($importData[8]))) != '-') ? trim($importData[8]) : NULL;
                            $country    = (!empty(strtolower(trim($importData[9]))) || trim(strtolower(trim($importData[9]))) != '-') ? trim($importData[9]) : NULL;
                            $zipCode    = (!empty(trim($importData[10])) || trim($importData[10]) != '-') ? trim($importData[10]) : NULL;
                    ...
                    $csv_file_path = Storage::disk(Config::get('constant.FILESYSTEM_CLOUD'))->path(Config::get('constant.IMPORT_CONSUMER_USER_CSV_UPLOAD_PATH').$filename);
	...
	/*
		@Author : Spec Developer
		@Desc   : Export consumer user into csv.
		@Output : \Illuminate\Http\Response
		@Date   : 11/03/2022
	*/

	public function exportConsumerUsers(Request $request){

		if($request->action == "exportConsumerUser"){

			//$users = User::select('name','email','phone','date_of_birth','gender')->where('role',2)->orderby('id','desc')->get();

			$delimiter = ",";
			$fileName = 'consumer_users.csv';
			$filePath = Storage::disk(Config::get('constant.FILESYSTEM_CLOUD'))->path(Config::get('constant.EXPORT_CONSUMER_USER_CSV_UPLOAD_PATH').$fileName);
			...
			foreach ($results as $users) {
				...
				$date_of_birth =!empty($users->date_of_birth) ?  $users->date_of_birth : '-';
				$gender = $users->gender;
