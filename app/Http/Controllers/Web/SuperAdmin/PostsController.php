...
class PostsController extends Controller
{
	...
	/*
		...
		@Desc   : Fetch super admin's post listing.
	...
	*/
    ...
    public function myPosts(Request $request){
		...
		$data['page_js'] = array(
            'backend/assets/superadmin/js/posts.js'
        );
       ...
       if ($request->ajax()) {
            ...
            $totalRecords = UserPost::with('user:id,user_name,role')->withCount('comments')
            ->withCount('likes')
            ->withCount('flaggedPost')
            ->where('user_id', Auth::user()->id)
            ->select('count(*) as allcount')->count();

            $postObj = UserPost::with('user:id,user_name,role')->withCount('comments')
            ->withCount('likes')
            ->withCount('flaggedPost')
            ->where('user_id', Auth::user()->id)
            ...
            foreach($postData as $key => $row) {
                ...
                $data_arr[$key]->comments_count = ($row->comments_count > 0) ? '<a href="'.url('superadmin/posts/comments/'.$row->id).'"><span class="badge counter-bg">'.$row->comments_count.'</span></a>' : '<span class="badge counter-bg disabled">'.$row->comments_count.'</span>';
                $data_arr[$key]->likes_count = ($row->likes_count > 0) ? '<a class="viewLikesRecord" href="javascript:void(0)" data-id="' . $row->id . '" data-url="'.url('superadmin/posts/view-post-likes').'" title="View"><span class="badge counter-bg">'.$row->likes_count.'</span></a>' : '<span class="badge counter-bg disabled">'.$row->likes_count.'</span>';
                $data_arr[$key]->flagged_post_count = ($row->flagged_post_count > 0) ? '<a class="viewFlaggedRecord" href="javascript:void(0)" data-id="' . $row->id . '" data-url="'.url('superadmin/posts/view-post-flagged').'" title="View flaggd"><span class="badge flag-counter-bg">'.$row->flagged_post_count.'</span></a>' : '<span class="badge flag-counter-bg disabled">'.$row->flagged_post_count.'</span>';
                ...
                $btn = '';
                $btn .= '<a class="viewRecord ml-2 mr-2" href="javascript:void(0)" data-id="' . $row->id . '" data-url="'.url('superadmin/my-posts/view').'" title="View"><i class="fa fa-search-plus fa-action-view"></i></a>';
                $btn .= '<a class="editRecord ml-2 mr-2" href="'.url('superadmin/my-posts/edit').'/'.$row->id.'" title="Edit"><i class="fa fa-pencil fa-action-edit"></i></a>';
                $btn .= '<a class="delete ml-2 mr-2" href="javascript:void(0);" data-id="' . $row->id . '" data-url="'.url('superadmin/my-posts/destroy/').'" title="Delete" data-target="#DeleteModal"><i class="fa fa-trash fa-action-delete"></i></a>';
        ...
        return view('superadmin.posts.mypost-list',$data);
    }

    /*
		@Author : Spec Developer
		@Desc   : Fetch user's post listing.
		@Output : \Illuminate\Http\Response
		@Date   : 08/03/2022
	*/
    ...
    public function usersPosts(Request $request){
        ...
        if ($request->ajax()) {
            ...
            foreach($postData as $key => $row) {
                ...
                if ($row->user) {
                    ...
                    $data_arr[$key]->user_id = '-';
                }
                $data_arr[$key]->status = ($row->status == 0) ? '<label class="label label-danger">Deactive</label>' : '<label class="label label-success">Active</label>';
                $btn = '';
                $btn .= '<a class="viewRecord ml-2 mr-2" href="javascript:void(0)" data-id="' . $row->id . '" data-url="'.url('superadmin/posts/view').'" title="View"><i class="fa fa-search-plus fa-action-view"></i></a>';
                if ($row->user && Auth::user()->role == $row->user->role) {
                    $btn .= '<a class="editRecord ml-2 mr-2" href="'.url('superadmin/my-posts/edit').'/'.$row->id.'" title="Edit"><i class="fa fa-pencil fa-action-edit"></i></a>';
                    $btn .= '<a class="delete ml-2 mr-2" href="javascript:void(0);" data-id="' . $row->id . '" data-url="'.url('superadmin/my-posts/destroy/').'" title="Delete" data-target="#DeleteModal"><i class="fa fa-trash fa-action-delete"></i></a>';
                } else {
                    $btn .= '<i class="fa fa-pencil fa-action-edit edit-bg disabled ml-2 mr-2"></i>';
                    $btn .= '<i class="fa fa-trash fa-action-delete delete-bg disabled ml-2 mr-2"></i>';
                }
                $data_arr[$key]->action = $btn;
            }
     ...
     return view('superadmin.posts.list',$data);
    }
	...
	*/
    ...
    public function flaggedPosts(Request $request){
		...
		$data['page_js'] = array(
            'backend/assets/superadmin/js/flagged_posts.js'
        );
        $data['extra_css'] = array(
            'plugins/table/datatable/datatables.css',
            'plugins/table/datatable/datatables.css',
       ...
       if ($request->ajax()) {
        ...
        return view('superadmin.posts.flaggedpost-list',$data);
    }
	...
	*/
    public function add(){
		$data['page_title'] = 'Add My Post';
        $data['page_js']    = array(
            'backend/assets/superadmin/js/add_posts.js'
        );
        $data['extra_css'] = array(
        ...
        return view('superadmin.posts.mypost-add',$data);
	}
	...
	*/
    public function store(Request $request){
       ...
       try {
			...
			$post_content	= trim($request->get('post_content'));
		    ...
		    return redirect('superadmin/my-posts');
        } catch (\Exception $e) {
            // Log error message
		    ...
		    return redirect('superadmin/my-posts/add');
        }
    }
	...
	*/
    ...
    public function editPost($id){
        ...
        $data['page_title'] = 'Edit My Post';
        $data['page_js']    = array(
            'backend/assets/superadmin/js/edit_posts.js'
        );
        $data['extra_css'] = array(
            'plugins/bootstrap-switch/custom/css/bootstrap-switch.css',
            'plugins/autotag/css/jquery.atwho.css',
        ...
        $galleryRs = PostGallery::select('id','file_name','thumb_name','file_type','file_length','file_size', 'media_order')->where('post_id',$id)->orderby('media_order')->get();
        ...
        return view('superadmin.posts.mypost-edit',$data);
    }
    ...
    */
	...
	public function updatePost(Request $request){
        try {
            ...
            $validator = Validator::make($request->all(),$validationArr);
            if ($validator->fails()) {
                Log::info('Edit post by super admin :: message :' . $validator->errors());
                return redirect()->back()->withErrors($validator)->withInput();
            }
    ...
    */
    ...
    public function view(Request $request){
        ...
        // return $row['data'];
        // die;
		return view('superadmin.posts.view',$row);
    }
    ...
    public function archivePosts(Request $request){
		...
		if ($request->ajax()) {
            ...
            foreach($postData as $key => $row) {
                ...
                $data_arr[$key]->status = ($row->status == 0) ? '<label class="label label-danger">Deactive</label>' : '<label class="label label-success">Active</label>';
                $btn = '';
                $btn .= '<a class="viewRecord ml-2 mr-2" href="javascript:void(0)" data-id="' . $row->id . '" data-url="'.url('superadmin/posts/archive-view').'" title="View"><i class="fa fa-search-plus fa-action-view"></i></a>';
                $btn .= '<a class="restore ml-2 mr-2" href="javascript:void(0);" data-id="' . $row->id . '" title="Restore" data-url="'.url('superadmin/posts/restore').'" data-target="#restoreModal" ><i class="fa fa-refresh fa-action-restore"></i></a>';
                $btn .= '<a class="delete ml-2 mr-2" href="javascript:void(0);" data-id="' . $row->id . '" data-url="'.url('superadmin/posts/force-delete').'" title="Delete" data-target="#DeleteModal"><i class="fa fa-trash fa-action-delete"></i></a>';
                $data_arr[$key]->action = $btn;
            }
        ...
        return view('superadmin.posts.archive_list',$data);
    }
    ...
    */
    ...
    public function viewPostLikes(Request $request){
		...
		return view('superadmin.posts.viewLikes',$row);
    }
