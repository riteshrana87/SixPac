...
class SettingsController extends Controller
{
    ...
    /*
        ...
        @Desc   : Fetch super admin details for edit profile.
        @Date   : 18/02/2022
    */
	public function editProfile(){
		$data['page_title'] = 'Edit Profile';
		$data['page_js'] = array(
            'backend/assets/business/js/settings.js'
        );
        ...
        return view('business.settings.edit_profile',$data);
	}

    /*
        @Author : Spec Developer
        @Desc   : Update super admin details for edit profile.
        ...
        try {
            ...
            if (empty($request->file('avtar')) && !empty($request->get('avtarImg'))) {           
               ...
               $file = base64_decode($base64_str);                
                ...
                if ($gallery) {
                    $filePath = $this->businessThumbImagePath.$gallery->avtar;
            ...
            if (!empty($request->file('profile_photo')) && $request->file('profile_photo')->isValid()) {
                $params = [
                    ...
                    if (!empty($request->get('avtarImg'))) {
                        ...
                        $avtarFile = Storage::disk('public')->put($this->businessThumbImagePath.$fileName, $decodedFile);
		    ...
		    return redirect('business/settings/edit-profile');
        } catch (\Exception $e) {
            // Log error message
    ...
    */
    ...
    public function changePassword(){
        ...
        return view('business.settings.change_password',$data);
	}

    /*
        @Author : Spec Developer
        @Desc   : Change password for super admin user.
        @Output : \Illuminate\Http\Response
        @Date   : 18/02/2022
    */
	...
	public function updatePassword(Request $request){
		try {
            ...
            if (!Hash::check($data['current_password'], $user->password)) {
                ...
                return redirect('business/settings/change-password')->with('error', 'The current password does not match!');
            }
            else
            {
                if ($request['current_password'] == $request['new_password']) {
                    //return back()->with('error', 'Your new password should be different from current password!');
                    return redirect('business/settings/change-password')->with('error', 'Your new password should be different from current password!');
                }
                else
                {
                    $obj = User::find($id);
                    ...
                    return redirect('business/settings/change-password');
                }
            }
        } catch (\Exception $e) {
            // Log error message
     ...
     public function getCityList(Request $request){
        ...
        if (count($cities) > 0) {
            ...
            foreach ($cities as $city) {
                $result[$i]['value'] = $city->id;
                $result[$i]['text'] = $city->name;
                $i++;
            }
        }
