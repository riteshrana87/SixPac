...
var BusinessUsers = (function () {
    var manage = function () {
        $("#importUsers").hide();
        var table = $("#tblBusinessUsers").DataTable({
            processing: true,
            serverSide: true,
            ajax: superadmin_url + "/users/business-users",
            order: [[0, "desc"]],
            columns: [
                { data: "id", name: "id" },
                { data: "name", name: "name" },
        ...
        $("#exportBusinessUser").click(function () {
            $("#exportBusinessUser").html("Loading...").addClass("disabled");
            var url =
                superadmin_url + "/users/business-users/exportBusinessUsers";
            $.ajax({
                type: "POST",
                url: url,
                data: { action: "exportBusinessUser" },
                dataType: "binary",
                xhrFields: {
                    responseType: "blob",
                },
                success: function (result) {
                    ...
                    link.download = "business_users.csv";
    ...
    $("body").on(
        ...
        function (event, state) {
            ...
            var url = superadmin_url + "/users/business-users/changeStatus/";
            if (state) {
                $("#status_" + id).val(1);
            } else {
                $("#status_" + id).val(0);
    ...
    var add = function () {
        ...
        var form = $("form[name='frmAddBusinessUser']");
        $("#btnSubmit").click(function (e) {
            ...
            form.validate({
                ...
                messages: {
                    name: {
                        required: "Please enter business user name.",
                        minlength:
                            "Business name minimum length should be 4 character.",
                        maxlength:
                            "Business name maximum length should be 40 character.",
                    },
                    user_name: {
                    ...
                    company_name: {
                        ...
                        minlength:
                            "Company name minimum length should be 3 character.",
                        maxlength:
                            "Company name maximum length should be 70 character.",
                    },
                    company_url: {
                        url: "Please enter valid URL.",
                        minlength:
                            "Company name minimum length should be 10 character.",
                        maxlength:
                            "Company name maximum length should be 255 character.",
                    },
                    phone: {
                        ...
                        maxlength:
                            "Business name maximum length should be 15 digit.",
					...
					zipcode: {
						...
						minlength:	'ZIP or postal code minimum length should be 4 character.',
						maxlength:	'ZIP or postal code maximum length should be 10 character.',
					}, */
                    avtar: {
    ...
    var edit = function () {
        ...
        var form = $("form[name='frmEditBusinessUser']");
        ...
        /** Save business user profile picture code start here **/
        $("#avtar").hide();
        $("body").on("click", "#pf_edit, #pf_upload_icon", function () {
            $("#avtar").trigger("click");
        });
        /** Save business user profile picture code end here **/
    };
    ...
    var delete_record = function () {
        $("body").on("click", ".delete", function () {
            var id = $(this).attr("data-id");
            var url = superadmin_url + "/users/business-users/destroy/" + id;
    ...
    /** Import csv file for admin user code start here **/
    $("body").on("click", "#importBusinessUser", function () {
        $("#csv_file").trigger("click");
    });

    $("#csv_file").on("change", function () {
        $("#importBusinessUser").html("Loading...").addClass("disabled");
        $("#importUsers").submit();
    });
            ...
            success: function (data) {
                ...
                if (data == 1) {
                    Swal.fire({
                        title: "Success!",
                        icon: "success",
                        text: "Business user data has been imported successfully!",
                        type: "success",
                        timer: 5000,
                    }).then((okay) => {
                        if (okay) {
                            window.location.href =
                                superadmin_url + "/users/business-users/";
                        }
                    });
                }
                if (data == 0) {
    ...
    /** Import csv file for business user code end here **/
