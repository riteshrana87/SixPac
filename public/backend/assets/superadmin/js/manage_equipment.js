...
function manage() {
    var table = $("#tblEquipment").DataTable({
        processing: true,
        serverSide: true,
        ajax: superadmin_url + "/get-fit/equipment",
        order: [[0, "desc"]],
        columns: [
            { data: "id", name: "id" },
...
function add() {
    ...
    var form = $("form[name='frmAddEditEquipment']");
    $("#btnSubmit").click(function (e) {
        ...
        form.validate({
            ...
            messages: {
                name: {
                    required: "The equipment is required.",
                    minlength:
                        "Equipment minimum length should be 4 character.",
                    maxlength:
                        "Equipment maximum length should be 40 character.",
                },
                //  getfit_id:{
                //     required: 'The getfit type is required.'
                // },
                icon_file: {
                    required: "Please select equipment icon.",
                },
            },
        });
        if (form.valid()) {
