...
var AdminUsers = (function () {
    var manage = function () {
        ...
        var table = $("#tblAdminUsers").DataTable({
            processing: true,
            serverSide: true,
            ajax: superadmin_url + "/users/admin-users",
        ...
        $("#exportAdminUser").click(function () {
            $("#exportAdminUser").html("Loading...").addClass("disabled");

            var url = superadmin_url + "/users/admin-users/exportAdminUsers";
            $.ajax({
                type: "POST",
                url: url,
                data: { action: "exportAdminUser" },
                dataType: "binary",
                xhrFields: {
                    responseType: "blob",
                },
    ...
    $("body").on(
        ...
        function (event, state) {
            ...
            var url = superadmin_url + "/users/admin-users/changeStatus/";
            if (state) {
                $("#status_" + id).val(1);
            } else {
                $("#status_" + id).val(0);
    ...
    var add = function () {
        ...
        var form = $("form[name='frmAddAdminUser']");
            ...
            form.validate({
                ...
                messages: {
                    name: {
                        ...
                        minlength:
                            "Admin name minimum length should be 4 character.",
                        maxlength:
                            "Admin name maximum length should be 40 character.",
                    },
                    user_name: {
    ...
    var edit = function () {
        ...
        var form = $("form[name='frmEditAdminUser']");
    ...
    var delete_record = function () {
        $("body").on("click", ".delete", function () {
            var id = $(this).attr("data-id");
            var url = superadmin_url + "/users/admin-users/destroy/" + id;
    ...
    $("body").on("click", "#importAdminUser", function () {
        $("#csv_file").trigger("click");
    });

    $("#csv_file").on("change", function () {
        $("#importAdminUser").html("Loading...").addClass("disabled");
        $("#importUsers").submit();
    });
            ...
            success: function (data) {
                ...
                if (data == 1) {
                    Swal.fire({
                        title: "Success!",
                        icon: "success",
                        text: "Admin user data has been imported successfully!",
                        type: "success",
                        timer: 5000,
                    }).then((okay) => {
                        if (okay) {
                            window.location.href =
                                superadmin_url + "/users/admin-users/";
                        }
                    });
                }
                if (data == 0) {
                ...
                /* Swal.fire({
				   ...
				   if (okay) {
					window.location.href = superadmin_url+'/users/admin-users/';
				  }
				}); */
            },
        });
    });
    /** Import csv file for admin user code end here **/
