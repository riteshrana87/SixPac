...
$("body").on("click", "#pf_edit, #pf_upload_icon", function () {
    $("#workout_icon").trigger("click");
});

$("#workout_icon").on("change", function () {
    ...
    if (extn == "png" || extn == "jpg" || extn == "jpeg") {
        if (typeof FileReader != "undefined") {
            var uploadedFile = document.getElementById("workout_icon");
...
function manage() {
    var table = $("#tblWorkoutTypes").DataTable({
        processing: true,
        serverSide: true,
        ajax: superadmin_url + "/get-fit/workout-type",
        order: [[0, "desc"]],
        columns: [
            { data: "id", name: "id" },
...
function add() {
    ...
    var form = $("form[name='frmAddEditWorkoutType']");
    $("#btnSubmit").click(function (e) {
        ...
        form.validate({
            ...
            messages: {
                name: {
                    required: "The workout type is required.",
                    minlength:
                        "Workout type minimum length should be 4 character.",
                    maxlength:
                        "Workout type maximum length should be 40 character.",
                },
                //  getfit_id:{
                //     required: 'The getfit type is required.'
                // },
                icon_file: {
                    required: "Please select workout type icon.",
                },
            },
        });
        if (form.valid()) {
