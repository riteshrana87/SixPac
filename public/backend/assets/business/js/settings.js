			...
			form.validate({
				...
				rules: {
					...
					email: {
						required: true,
						email: true,
						remote: business_url+'/settings/validateUserEmail?id='+user_id,						
					},
					phone: {
						required: true,
						minlength : 10,
						maxlength: 15,
						remote: business_url+'/settings/validateUserPhone?id='+user_id,						
					},
					date_of_birth: {
						required: true						
					},
					profile_photo: {
						required: function (element) {
							if($("#profile_photo").val() == "" && $("#pf_img").val() != ""){
								return false
							}
							else
							{
				...
				messages: {
					...
					company_name: {
						...
						minlength:	'Company name minimum length should be 3 character.',
						maxlength:	'Company name maximum length should be 70 character.',
					},
					company_url: {
						url: 'Please enter valid URL.',
						minlength:	'Company name minimum length should be 10 character.',
						maxlength:	'Company name maximum length should be 255 character.',
					},
					email: {
					...
					phone: {
						...
						maxlength:	'Admin name maximum length should be 15 digit.',
						...
						reader.onload = function(e) {
							...
							image.onload = function () {
								...
								if (width < 100 || width > 800) {
									callToaster('error','Profile photo width should be beetween 100px to 800px.');
								...
								else
								{
									...
									$("#profile-photo-preview").html("<img src='" +e.target.result +"' id='profile_photo_placeholder' width='185px' height='185px'>"+editBtn);
								}
							}
						}
					}
		...
		$("form[name='frmPassword']").validate({
			...
			messages: {
				...
				new_password: {
					...
					minlength:	"New password minimum legnth should be 8 character.",
					maxlength:	"New password maximum legnth should be 15 character.",
				},
				confirm_password: {
