...
$('body').on("click",".chk-exercise", function () {
    $(this).parents(".exercise__box").toggleClass("active");
});
...
$(document).on("click", "#addExercise", function (e) {
    $("#exerciseBody").html("");
    $("#exerciseModal").modal({ backdrop: "static", keyboard: false }, "show");
    submitcall(
        business_url + "/get-fit/workout/create-exercise",
        {},
        function (output) {
            $("#exerciseBody").html(output);
            $("input[data-bootstrap-switch]").each(function () {
                $(this).bootstrapSwitch("state", $(this).prop("checked"));
            });
        }
...
$("body").on("click", "#delete_img", function () {
    $("#exerciseImgPreview").hide();
    $("#poster_img").val("");
    $("#poster_img").next(".custom-file-label").html("Choose file");
});

$("body").on("click", "#deleteExeVideo", function () {
    $("#previewImg").attr("src", "");
    $("#video_thumb").val("");
    $("#exerciseVideo").val("");
    $("#ExevideoPreview").hide();
    $("#videoName").val("");
    $("#videoName").next(".custom-file-label").html("Choose file");
});

$("body").on("change", "#poster_img", function (e) {
    ...
    if (extn == "png" || extn == "jpg" || extn == "jpeg") {
        if (typeof FileReader != "undefined") {
            var uploadedFile = document.getElementById("poster_img");
                ...
                reader.onload = function (e) {
                    ...
                    image.onload = function () {
                        ...
                        if (width < 200 || width > 1000) {
                            ...
                            $("#exerciseImgPreview").html("");
                            $("#poster_img").val("");
                            return false;
                        } else {
                            $("#exerciseImgPreview").show();
                            $self.next(".custom-file-label").html(fileName);
                            $("#exerciseImgPreview").css({
                                display: "flex",
                                "justify-content": "center",
                            });
                            var editBtn =
                                '<a href="javascript:void(0)" id="delete_img" class="deleteMedia"><i class="fa fa-trash"></i></a>';
                            $("#exerciseImgPreview").html(
                                "<img src='" +
                                    e.target.result +
...
$("body").on("change", "#videoName", function (e) {
    var fileName = e.target.files[0].name;
    var imgPath = $(this)[0].value;
    var $self = $(this);
    $(this).next(".custom-file-label").html("Choose file");
...
$("body").on("click", "#saveExerciseBtn", function () {
    var form = $("form[name='frmAddEditExercise']");
    // console.log('form=>',form.serialize()); return false;
    form.validate({
        ignore: [],
        rules: {
            name: {
    ...
    if (form.valid()) {
        $.ajax({
            type: "POST",
            url: business_url + "/get-fit/workout/add-exercise",
            ...
            success: function (result) {
                if (result.status===true) {
                    $('.workout-exercises').prepend(result.data);
                    $("#exerciseModal").modal('hide');
                    showToaster("success", result.message);
                } else {
                    showToaster("error", result.message);
                }
            },
        ...
        // submitcall(business_url+'/get-fit/workout/add-exercise',form.serialize(), function (output) {
        //     console.lotg('output=>',output)
        // });
    }
});
...
$("body").on("change", "#poster_image", function (e) {
    var fileName = e.target.files[0].name;
    var imgPath = $(this)[0].value;
    var $self = $(this);
    $(this).next(".custom-file-label").html("Choose file");
...
$("body").on("change", "#video_name", function (e) {
    var fileName = e.target.files[0].name;
    var imgPath = $(this)[0].value;
    var $self = $(this);
    $(this).next(".custom-file-label").html("Choose file");
...
function add() {
    ...
    var form = $("form[name='frmAddEditWorkout']");
    ...
    $("body").on("click", "#btnSubmit", function (e) {
        ...
        form.validate({
            ...
            messages: {
                name: {
                    required: "Please enter workout title.",
                    minlength:
                        "Workout title minimum length should be 4 character.",
                    maxlength:
                        "Workout title maximum length should be 200 character.",
                },
                overview: {
                    required: "Please enter workout overview.",
                    minlength:
                        "Workout overview minimum length should be 4 character.",
                    maxlength:
                        "Workout overview maximum length should be 500 character.",
                },
                poster_image: {
                    required: "Please select poster image.",
                },
                video_name: {
                    required: "Please select workout video.",
                },           
                workout_type_id: {
                    required: "Please select workout type.",
                },
             ...
             errorPlacement: function (error, element) {
                if (["workout_type_id","duration_id","equipment[]","body_part[]","fitness_level[]","exercise[]"].includes(element.attr("name"))) {
                    var attrName = element.attr("data-name");
                    error.css("padding-left", "12px");
                    error.insertAfter($("#err_" + attrName));
                } else {
...
function manage() {
    var table = $("#tblExercises").DataTable({
        processing: true,
        serverSide: true,
        ajax: business_url + "/get-fit/workout",
        order: [[0, "desc"]],
        columns: [
            { data: "id", name: "id" },
