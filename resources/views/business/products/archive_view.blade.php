				...
				<div class="row preview-image">
					@if(count($media) > 0)
						@foreach($media as $proMedia)
							@if($proMedia->file_type != "video/mp4" && $proMedia->file_type != "mp4")
								<div class="col-md-4">
									<img src="{{ $proMedia->file_url }}" width="200px" height="200px">
								</div>
							@endif
							@if($proMedia->file_type == "video/mp4" || $proMedia->file_type == "mp4")
								<div class="col-md-4">
									<video src="{{ $proMedia->file_url }}" type="{{ $proMedia->file_type }}" width="200px" height="200px"></video>
								</div>
							@endif
						@endforeach
					@else
